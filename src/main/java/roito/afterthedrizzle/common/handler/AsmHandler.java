...
{
    public static float getSeasonCelestialAngle(long worldTime, float partialTicks, IWorld world)
    {
        return getCelestialAngle(getSolarAngelTime(worldTime, partialTicks, world), partialTicks);
    }

    public static int getSolarAngelTime(long worldTime, float partialTicks, IWorld world)
    {
        if (world instanceof World)
        {
            return ((World) world).getCapability(CapabilitySolarTermTime.WORLD_SOLAR_TIME).map(data ->
            {
                int dayTime = SolarTerms.get(data.getSolarTermIndex()).getDayTime();
